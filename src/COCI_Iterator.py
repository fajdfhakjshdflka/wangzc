...
from src.COCI_Manager import COCIManager
...
class COCIIterator:
    def __init__(self,
        ...
        self.meta_state = {}
        self.iter_list = [0]  # iter completion time
        self.loss_list = [-1]
        ...
        self.COCIManager = COCIManager(model_name, dataloader, **ck_kwargs)
    ...
    def COCI_save_dynamic(self):
        ...
        if self.para_profile_flag is True:
            ...
            t = time.time()
            # Adjusting checkpoints, it cannot be taken within iteration
    ...
    def optimizer_step(self, loss, model, optimizer):
        ...
        if hasattr(model, 'state_dict'):
            ...
            print("No state_dict() method exposed in object{}".format('model'))

        if hasattr(optimizer, 'state_dict'):
            self.COCIManager.ck.destination['optimizer'] = optimizer
        else:
            print("No state_dict() method exposed in object{}".format('optimizer'))
    ...
    def recovery(self,
                 gpu=0):
        last_metadata_path, flag, dataloader = self.COCIManager.recovery(gpu=gpu)
        if flag is True:
            self.dataloader = dataloader
            ...
            if metadata_dict['is_last_one'] is False:
                ...
                theta_1 = self.parameters['a']
                theta_2 = self.parameters['b']
                print('Last training is over!!! It is a new training.')               
                print(f'The theta_1 is {theta_1}, and theta_2 is {theta_2}.')
                print('Please redefine the dataloader!!!')
                return True, 0
        else:
            False, None, None
    ...
    def _get_meta_state(self):
        ...
        self.meta_state['iter_list'] = self.iter_list
        self.meta_state['loss_list'] = self.loss_list
    ...
    def get_checkpoint(self):
        model, optimizer = self.COCIManager._get_checkpoint()
        self.checkpoints['model'] = model
        self.checkpoints['optimizer'] = optimizer
        self.checkpoints['epoch'] = self.meta_state['epoch']
        self.checkpoints['ck_time'] = self.meta_state['ck_time']
        self.checkpoints['iter_list'] = self.meta_state['iter_list']
        self.checkpoints['loss_list'] = self.meta_state['loss_list']
